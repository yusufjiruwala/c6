sap.ui.jsfragment("bin.forms.clinic.post", {
...
//                that.qv.mLctb.getColByName("ORD_NO").mHideCol = true;
                ...
                switch (oc) {
                    ...
                    case 111:
                        sql = "update order1 set posted_date=to_date(to_char(sysd,'dd/mm/rrrr'),'dd/mm/rrrr') " +
                            ...
                            "X_POST_APPOINT(" + on + "); update order1 set approved_by='" + usr + "' where ord_code=111 and ord_no=" + on + ";";
                        break;
                    default:
                        break;
                }
    ...
    printAllSel: function () {
        ...
        if (dt.ret = "SUCCESS")
            for (var i in ocs)
                Util.doXhr("report?reportfile=rptVouCL" + ocs[i], true, function (e) {
                    if (this.status == 200) {
