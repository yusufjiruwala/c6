...
//            this.mJsonObject = JSON.parse(jsonStr);
        ...
        QueryView.prototype.buildJsonData = function () {
            ...
            for (var i = 0; i < Util.nvl(o, []).length; i++) {
                ...
                for (var v in o[i]) {
                    ...
                    if (typeof (o[i][v]) == "number") {
                        v
                        if (this.mLctb.getColByName(vv).mSummary == "SUM") {
                        ...
                        } else if (this.mLctb.getColByName(vv).mSummary == "LAST") {
                            footerg[v] = o[i][v];
                            footer[v] = o[i][v];
                        }
                        else {
                        ...
                        // footer[v] = null;
                        // footerg[v] = null;
                        if (v != "_rowid" && this.mLctb.getColByName(vv).getMUIHelper().display_format === "SHORT_DATE_FORMAT") {
                            if (Util.nvl(o[i][v], "").length > 0) {
                                var dt = new Date(o[i][v]);
                                o[i][v] = sf.format(dt);
                            }
            ...
            for (var fv in footer)  // formating...
            {
                ...
                if (fv != "_rowid" && this.mLctb.getColByName(fvv).getMUIHelper().display_format === "MONEY_FORMAT")
                    footer[fv] = df.format(footer[fv]);
                if (fv != "_rowid" && this.mLctb.getColByName(fvv).getMUIHelper().display_format === "QTY_FORMAT")
...
//tree buildjson data
        ...
        QueryView.prototype.colorRows = function () {
            ...
            for (var i = 0; i < rowCount - 1; i++) {
                if (this.getControl().getRows()[i].getCells()[0] == undefined)
                    continue;
